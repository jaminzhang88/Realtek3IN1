			...
			struct ayla_tlv *val)
{
	...
	conf_tlv_fmt(vbuf, sizeof(vbuf), val);
        ...
        //增添以获取默认设置的region值
        if(nbuf[0]=='c'&&nbuf[1]=='l'&&nbuf[14]=='r'&&nbuf[15]=='e'){
            if(vbuf[1]=='S'&&vbuf[2]=='A'){
               printf("1:%s\n",vbuf);
               flag_region_which=1;
              }else if(vbuf[1]=='U'&&vbuf[2]=='S'){
               printf("2:%s\n",vbuf);
               flag_region_which=2;
             }else if(vbuf[1]=='E'&&vbuf[2]=='U'){
               printf("3:%s\n",vbuf);
               flag_region_which=3;
            }
        }
...
void conf_cli(int argc, char **argv)
{
	...
	if (argc == 2 && !strcmp(argv[1], "airkiss")) {
	        char *argv[]={"wifi","setup_mode","airkiss","enable"};
                adw_wifi_cli(4, argv);
		char *argv0[] = { "conf", "save" };
                conf_cli(2, argv0);//s保存
	        char *argv1[] = { "reset" };
                demo_reset_cmd(1, argv1);
	}
	       ...
	       char *argv1[] = { "conf", "save" };
               conf_cli(2, argv1);//保存
               //退出setup_mode
	       char *argv2[] = { "setup_mode", "disable" };
	       demo_setup_mode_cmd(3,argv2);
	}
...
//设置参数
        //OEM
       if (argc == 4 && !strcmp(argv[1], "oem")) {
	        int len=strlen(argv[2]);
	        if(len>20){
	           printf("\npara set err\r\n");
	        }else{
	            OEM_SET(argv[2],argv[3],len);
	            printf("\npara set ok\r\n");
	        }
        }
     //OEM_MODEL
     if (argc == 4 && !strcmp(argv[1], "oemmodel")) {
	        int len=strlen(argv[2]);
	        if(len>20){
	           printf("\npara set err\r\n");
	        }else{
	            OEMMODEL_SET(argv[2],argv[3],len);
                printf("\npara set ok\r\n");
	        }
       }
      //OEM_KEY
      if (argc == 4 && !strcmp(argv[1], "oemkey")) {
	        int len=strlen(argv[2]);
	        if(len>50){
	           printf("\npara set err\r\n");
	        }else{
	            OEMKEY_SET(argv[2],argv[3],len);
	            printf("\npara set ok\r\n");
	        }
        }
      //DSN
       if (argc == 4 && !strcmp(argv[1], "dsn")) {
	        int len=strlen(argv[2]);
	        if(len>20){
	           printf("\npara set err\r\n");
	        }else{
	            DSN_SET(argv[2],argv[3],len);
	            printf("\npara set ok\r\n");
	        }
       }
     ...
     //REGION
     if (argc == 3 && !strcmp(argv[1], "region")) {
	        int len=strlen(argv[2]);
	        if(len>10){
	           printf("\npara set err\r\n");
	        }else{
	           //写入自定义flash区域
	            REGION_SET(argv[2],len);
	            //写入系统自带flash区域
	            char *argv1[] = { "client","server","region",argv[2]};
      ...
      //设置默认使用region  conf sa/us/eu
        if (argc == 2 && !strcmp(argv[1], "sa")) {
            ...
            } else if (argc == 2 && !strcmp(argv[1], "us")) {
                int len=strlen(argv[1]);
                if(len>10){
                   printf("\npara set err\r\n");
                }else{
                    char *argv1[] = { "client","server","region" ,"us"};
                    vTaskDelay(300);
                   demo_client_cmd(4,argv1);
                   vTaskDelay(200);
                   printf("\npara set ok\r\n");
                }
	  } else if (argc == 2 && !strcmp(argv[1], "eu")) {
	        int len=strlen(argv[1]);
                if(len>10){
                   printf("\npara set err\r\n");
                }else{
                   char *argv1[] = { "client","server","region" ,"eu"};
                   vTaskDelay(300);
                   demo_client_cmd(4,argv1);
                   vTaskDelay(200);
                   printf("\npara set ok\r\n");
          ...
          if (argc == 2 && !strcmp(argv[1], "get")) {
               printf("  flag_region_which:%d\n",flag_region_which);
          }


           //真正开始连云
           if (!strcmp(argv[1], "sar")) {
